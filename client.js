document.addEventListener("DOMContentLoaded", () => {
  ...
  let currentSessionId = `sessao_${Date.now()}_${Math.random()
    .toString(36)
    .substring(7)}`;
  ...
  async function registrarAcessoBotParaRanking(botId, nomeBot) {
    try {
      ...
      const response = await fetch(
        `${backendUrl}/api/ranking/registrar-acesso-bot`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  function resetChat() {
    ...
    addMessage(
      "MUDA MUDA MUDA! Você ousa falar com o grande Dio-sama! Faça sua pergunta, mortal insignificante! Posso até mesmo revelar informações sobre o clima e horário de qualquer lugar do mundo, se for digno da minha atenção!",
      "bot"
    );
  }
  ...
  window.addEventListener("load", async () => {
    ...
    addMessage(
      "WRYYY! Você ousa se aproximar de mim, Dio-sama? Muito bem, mortal... Faça sua pergunta e talvez eu conceda minha sabedoria suprema! Posso até mesmo revelar os segredos do clima e do tempo de qualquer lugar do mundo!",
      "bot"
    );
  });
